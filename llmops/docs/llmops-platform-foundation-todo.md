...
- 模块四 Files：已新增直接使用的 `files` 表，记录 `storage_provider + file_path`，不落库绝对 URL；上传、列表、重命名、移动、递归软删除、下载 URL、知识库选择文件已接入；页面已补齐类网盘管理，包括面包屑、类型/来源筛选、列表/平铺视图、批量移动/删除。
...
- [x] 文件列表支持类云盘管理，包括面包屑、类型/来源筛选、列表/平铺视图、批量移动/删除。

...
- 通义千问配置已按阿里百炼官方模型页更新为 Max/Plus/Flash 组合：`qwen3.7-max`、`qwen3.7-plus`、`qwen3.6-flash`。
- 模块四 Files：已新增直接使用的 `files` 表，记录 `storage_provider + file_path`，不落库绝对 URL；上传、分页列表、重命名、移动、递归软删除、批量移动/删除、目录树、下载 URL、知识库选择文件已接入；页面已补齐类网盘管理，包括面包屑、类型/来源筛选、列表/平铺视图、批量移动/删除。
...
- Agent 前置追加：Router Manager 后端已支持 `input_file_ids` 作为 `WorkerInvocation.context.input_files`；小型文本文件会内联内容预览给 ReAct Worker；`WorkerResult.artifacts` 已支持文本产物登记到 `files.source=agent`，并通过 `ArtifactRef` 传递给后续 step。

本轮明确不做：

- Agent 任务页 UI 展示输入文件、产物文件和执行日志。
- 完整二进制产物生成、预览、清理和 sandbox 产物同步。
- LLM 模型价格计算；当前只保留对话中的 token 使用数量记录，不新增价格字段。
- COS/OSS SDK 真实上传、读取和签名下载；当前仅支持配置保存、脱敏和 URL 拼接。

## 1.1 下一阶段待办

当前 1-4 平台底座 MVP 已具备进入 Agent 工作的前置条件。后续待办按优先级拆分如下：

### Agent 前置与 Runtime 接入

- [x] 文件作为 Agent task 输入：Router Manager 后端支持 `input_file_ids`，并把文件引用写入 `WorkerInvocation.context.input_files`；小型文本文件内联内容预览。
- [x] Agent 产物登记为 Artifact：`WorkerResult.artifacts` 中带 `metadata.content/text` 的文本产物会写入文件资产层，标记 `source=agent`；已有 `file_id` 产物会做账号级校验后补齐文件元信息。
- [x] 引入 `ArtifactRef`：Worker 之间通过 `WorkerResult.artifacts -> WorkerInvocation.context.artifacts` 传递文件引用，不直接传二进制。
- [ ] Agent 任务页展示输入文件、产物文件和执行日志。

### 文件与存储产品化

- [x] 文件列表分页，避免当前用户文件多时一次拉全量。
- [x] 后端批量移动 / 批量删除接口，避免前端多次单条请求。
- [x] 独立目录树接口，避免移动弹窗递归请求目录。
- [ ] COS/OSS SDK 真实接入：实现云存储上传、读取和签名下载。
- [ ] 文件预览增强：PDF、Office、文本、图片等按类型预览。
- [ ] 文件去重或版本策略：处理同名、同 hash 文件的重复保存。

### 治理与企业化

- [ ] LLM 模型价格 / 成本统计。
- [ ] 模型调用审计：按用户、应用、工作流、Agent 任务记录调用明细。
- [ ] 企业多租户 / RBAC / workspace 级资源隔离。
- [ ] 独立 `CredentialVault`：统一管理模型密钥、工具密钥、存储密钥版本和轮换。
...
- [x] 文件列表支持类云盘管理，包括面包屑、类型/来源筛选、分页、列表/平铺视图、批量移动/删除。
- [x] 文件上传统一走 storage driver。
- [x] 文件可作为知识库来源。
- [x] 文件可作为 Agent task 输入（后端 Runtime 协议已接入，UI 待产品化）。
- [x] Agent 产物登记为 Artifact（文本产物和已有 file_id 引用已接入，完整二进制/sandbox 产物待产品化）。
...
- 通义千问系统模型源已切换到新系列，运行时允许 `enable_thinking`、`thinking_budget` 参数透传。

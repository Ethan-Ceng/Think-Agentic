...
- [ ] 文件作为 Agent task 输入：Agent 任务创建、调试和执行时可选择 `files` 资产作为输入上下文。
- [ ] Agent 产物登记为 Artifact：Agent/Worker 输出的报告、表格、图片、结果包登记到文件资产层，标记 `source=agent` 或更细的产物来源。
- [ ] 引入 `ArtifactRef`：Worker 之间传递文件引用，不直接传二进制；Artifact 可作为后续步骤输入。

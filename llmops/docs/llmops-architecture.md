...
- WorkerAgent 可以返回 `worker_plan_feedback_v1`，向 Planner/Router 建议更新剩余计划、提前完成或等待用户补充信息；它只能反馈，不能直接修改全局计划。
...
- 校验和落库计划。
- 顺序调度 Worker。
- 在 Worker 成功反馈改变执行路径时，受控更新剩余未执行步骤。
- 汇总 Worker 结果。
...
- 成功反馈驱动计划更新 `RouterPlannerAgent.update_plan_from_feedback()` 已落地：Worker 成功后若返回 `plan_feedback.needs_plan_update`，Router 会追加新 `AgentPlan/AgentStep`，旧计划标记 `superseded`，新计划重新执行 schema、绑定、依赖和 capability preflight。
- Step 执行契约已进入 `RouterPlanStep`、`AgentStep.input_json`、`WorkerInvocation.task` 和任务详情返回，包含 `expected_output`、`success_criteria`、`required_artifacts` 和 `handoff_context`。
- 轻量等待已收敛为 Task `waiting` 状态；Worker 可通过 `WorkerResult.status = waiting_user` 发出暂停信号，Router 只把 Task 置为 `waiting`，等待类型、缺失信息和恢复策略写入 `wait.user.requested` trace。Step 保持运行上下文，WorkerCall 保存本次结果，不再扩散三层 `waiting_user` 状态。
- v6-lite 运行分析已基于现有任务表和 trace 落地只读聚合接口：`GET /apps/{app_id}/agent-tasks/metrics`，覆盖成功率、等待数、replan/plan update、Worker 排行、错误码和 missing_info。
...
4. v2.5 已吸收 `agentic` 的执行反馈循环经验：Worker 成功反馈可触发受控 plan update，仍只追加新计划、只执行未完成工作，并保留 plan diff 和 trace。
5. v2.4 后续再实现 A2A 外部 Worker：新增 `a2a_agents` 表，`agents.target_ref_type = a2a_agent` 指向该表，`WorkerRuntime` 增加 text `message/send` executor。

...
2026-06-04 V2.2 / V2.3 设计定稿：

- V2.2 只做 A2A 外部 WorkerAgent：新增 `a2a_agents`，同步 Agent Card，映射为 `target_ref_type = "a2a_agent"` 的 WorkerAgent，接入 `WorkerRuntime` text `message/send` executor。
- V2.2 不支持文件上传、图片二进制转发、streaming、push notification、远程回调或长轮询任务；必须先落 URL/SSRF、超时、响应体大小和凭据脱敏边界。
- V2.3 只做一次动态重规划：Worker 失败或 preflight 能力不匹配时，Planner 可重写未执行 step；已完成 step 不可改写，新计划仍必须再次 validate 和 preflight。
- V2.3 不新增表，优先复用多条 `AgentPlan`、新 `AgentStep`、`plan_json.replan` 和 `TraceEvent` 表达原计划、新计划、失败原因和改派记录。

下一步实施顺序：

1. v2.2-1：`a2a_agents` 迁移、模型、服务、URL/SSRF 安全校验和 Agent Card discover/sync。
2. v2.2-2：A2A Agent 映射为外部 WorkerAgent，并复用现有 Planner `AgentBinding`。
3. v2.2-3：`A2AWorkerExecutor` 和 `WorkerRuntime` 派发，支持 text `message/send`，归一化 `WorkerResult`。
4. v2.2-4：前端添加/同步/测试/绑定 A2A Worker，任务页展示 A2A call trace。
5. v2.2-5：A2A 安全、协议错误、凭据缺失和历史快照验收。
6. v2.3-1：`RouterPlannerAgent.update_plan()`、replan input schema 和 prompt。
7. v2.3-2：失败触发、改派、最多一次重规划和新计划二次校验。
8. v2.3-3：任务页原计划/新计划/replan trace 展示。
9. v2.3-4：完整 V2 回归，覆盖内部 Worker、A2A Worker、能力缺失、Worker 失败和历史任务回放。

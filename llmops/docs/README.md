...
1. v2.1 手工验收记录：用真实 Planner/Worker 验证能力摘要刷新、routing policy 保存、preflight 阻断和任务页展示，并记录 session/task id。
2. v2.2：A2A Agent 注册、Agent Card 同步、SSRF/凭据安全、外部 WorkerAgent 映射、text `message/send` executor。
3. v2.3：Worker 失败或能力不匹配后动态重规划，展示原计划、新计划和改派记录。

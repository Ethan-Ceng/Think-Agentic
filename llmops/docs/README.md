...
2026-06-06 当前项目状态与优化总结：

- 自主性 Agent 整体应用设计已经大体成型，当前系统不再只是单轮 AI 应用调试，而是具备 PlannerAgent、WorkerAgent、Task、Plan、Step、WorkerCall、TraceEvent 和 RuntimeEvent 展示的多 Agent 编排底座。
- Planner/Worker 分工已经明确：PlannerAgent 只负责目标理解、计划生成、计划更新、Worker 选择和最终汇总；WorkerAgent 只执行当前 step，不修改全局计划。
- 发布态 Planner 已补齐共用 runtime 入口：`stream_planner_run` 成为 debug、web-app、OpenAPI 的共享 Planner 执行入口；原 `stream_planner_debug_run` 只保留为兼容包装。
- 独立 ReActAgent 抽象已补齐：Planner 执行 step 时通过 `ReActAgent.execute_step()` 调用 WorkerRuntime，后续 Worker 内部 ReAct、A2A executor、MCP/sandbox executor 可继续沿这个抽象扩展。
- Wait/resume 后端闭环已补齐：Task/Step 等待状态统一收敛为 `waiting`；web-app 和 OpenAPI 支持 `resume_task_id`，同一 conversation 存在 waiting task 时也可以自动恢复执行。
- 事件展示已从 debug 扩展到发布态 chat：实时 SSE 保留原 `agent_thought`，同时新增 `runtime_event`；历史消息可以回放 plan、step、tool、wait、error、done 等面向用户的运行事件。
- 统一 Event 存储访问边界已补齐：新增 `RuntimeEventStoreService`，以 `TraceEvent` 作为任务运行事实源，普通 chat 的 `MessageAgentThought` 只作为兼容回放源。
- 任务页和 analysis 页面已有运行分析基础：能查看 Task/Plan/Step/WorkerCall/TraceEvent、运行指标、等待数、失败数、Worker 调用和 trace 统计；本轮没有再新增独立 analysis 页面，而是保留在现有 analysis/任务分析体系内。
- debug 专用入口暂不移除。当前正确状态是“保留调试视图，内部复用 shared runtime”；等发布态 Planner、resume、运行事件回放和前端显式恢复操作稳定后，再清理重复命名或旧私有逻辑。

2026-06-06 自动化和 Docker 验证：

- 后端全量测试通过：`166 passed`。
- 后端 lint 通过：`uv run --project llmops/api ruff check llmops/api/app llmops/api/tests`。
- 前端类型检查通过：`pnpm type-check`。
- 前端生产构建通过：`pnpm build`。
- Docker 镜像已重建并启动：`llmops-api`、`llmops-celery`、`llmops-ui` 正常运行。
- API 健康检查通过：`/healthz`、`/readyz` 返回 `ok`。
- UI 入口可访问：`http://localhost:3100` 返回 200。

当前仍建议继续完善的部分：

- 前端 Wait/resume 产品化：在 WaitEvent 卡片上展示缺失字段、等待原因和“继续执行”操作，并显式传递 `resume_task_id`。
- PlanEvent 生命周期继续细化：把“正在规划、计划生成、计划更新、计划完成”做成更独立的 plan 展示层，而不是只依赖 tool/action 转换。
- Resume 并发和幂等：增加 `resume_idempotency_key` 或基于 `task.version` 的乐观锁，避免多端或重复点击造成重复恢复。
- 统一事件表可以作为后续 v6 方向：当前已通过 store facade 收拢读取边界；如需严格 append-only event log，可后续新增 `runtime_events` 表并迁移 TraceEvent 写入。
- 端到端验收继续补齐：真实 Planner app 的 `wait -> 用户补充 -> resume -> done` 场景、发布态 web-app 场景和 OpenAPI 场景建议增加 E2E 自动化。

import { del, get, post, ssePost } from '@/utils/request'
...
export const stopAssistantAgentChat = (task_id: string) => {
  return post<BaseResponse<any>>(`/assistant-agent/stop/${task_id}`)
}
...
export const deleteAssistantAgentConversation = () => {
  return del<BaseResponse<any>>(`/assistant-agent/conversation`)
}

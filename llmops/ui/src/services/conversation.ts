...
export const deleteConversation = (conversation_id: string) => {
  return del<BaseResponse<any>>(`/conversations/${conversation_id}`)
}

// 删除特定会话下的指定消息
export const deleteMessage = (conversation_id: string, message_id: string) => {
  return del<BaseResponse<any>>(`/conversations/${conversation_id}/messages/${message_id}`)
}
...
export const updateConversationName = (conversation_id: string, name: string) => {
  return put<BaseResponse<any>>(`/conversations/${conversation_id}/name`, { body: { name } })
}

// 置顶或取消置顶某个会话
export const updateConversationIsPinned = (conversation_id: string, is_pinned: boolean) => {
  return put<BaseResponse<any>>(`/conversations/${conversation_id}/is-pinned`, {
    body: { is_pinned },
  })
}

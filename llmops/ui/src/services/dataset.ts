...
export const getDocumentsStatus = (dataset_id: string, batch: string) => {
  return get<GetDocumentsStatusResponse>(`/datasets/${dataset_id}/documents/status/${batch}`)
}
...
export const deleteSegment = (dataset_id: string, document_id: string, segment_id: string) => {
  return del<BaseResponse<any>>(`/datasets/${dataset_id}/documents/${document_id}/segments/${segment_id}`)
}

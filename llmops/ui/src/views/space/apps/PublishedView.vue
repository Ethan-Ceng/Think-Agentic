...
<template>
  <div class="bg-white flex-1 w-full min-h-0 px-6 py-5">
    ...
    <!-- 发布渠道列表 -->
    <div v-loading="getPublishedConfigLoading" class="w-full">
      <table class="w-full">
        <thead>
        ...
        <tr class="border-b">
          <td class="py-3 px-4 w-2/3">
            <div class="flex items-center gap-2">
              <el-avatar :size="36" shape="square" class="bg-blue-100">
          ...
          <td class="py-3 px-4 w-1/12">
            <el-tag v-if="published_config?.web_app?.status !== 'published'" color="gray" bordered>
              <template #icon>
                <icon-minus-circle />
              </template>
              未发布
            </el-tag>
            <el-tag v-else color="blue" bordered>
              <template #icon>
          ...
          <td class="py-3 px-4">
            <div class="flex items-center gap-3">
              ...
              <!-- 右侧访问按钮 -->
              <el-button class="rounded-lg px-2">
                <template v-if="published_config?.web_app?.status !== 'published'">
                  立即访问
                </template>
                <template v-else>
        ...
        <tr class="border-b">
          <td class="py-3 px-4 w-2/3">
            <div class="flex items-center gap-2">
              <el-avatar :size="36" shape="square" class="bg-green-100">
          ...
          <td class="py-3 px-4 w-1/12">
            <el-tag v-if="wechat_config?.status !== 'configured'" color="gray" bordered>
              <template #icon>
                <icon-minus-circle />
              </template>
              未配置
    ...
    <!-- 微信公众号配置模态窗 -->
    <el-dialog
      v-model="wechatConfigModalVisible"
      header-class="hidden"
      :show-close="false"
      modal-class="rounded-xl w-[600px]"
      @close="handleCancelWechatConfigModal"
    >
      <!-- 顶部标题 -->
      ...
      <div class="py-4">
        <div class="flex flex-col gap-5">
          ...
          <div class="flex flex-col gap-2">
            ...
            <el-input v-model="wechatConfigForm.wechat_app_id" placeholder="请填写微信开发者ID" />
          </div>
          <!-- 开发者秘钥(AppSecret) -->
          <div class="flex flex-col gap-2">
            <div class="flex items-center gap-1 text-gray-700">
            ...
            <el-input v-model="wechatConfigForm.wechat_app_secret" placeholder="请填写微信开发者秘钥" />
          </div>
          <!-- 令牌(Token) -->
          <div class="flex flex-col gap-2">
            <div class="flex items-center gap-1 text-gray-700">
            ...
            <el-input v-model="wechatConfigForm.wechat_token" placeholder="请填写微信公众号令牌" />
          </div>
        </div>
      </div>
      <!-- 底部按钮 -->
      <div class="flex items-center justify-between">
        <div class=""></div>
        <el-space :size="16">
          <el-button class="rounded-lg" @click="handleCancelWechatConfigModal">取消</el-button>
          <el-button

...
const handleSelectWorkflow = (idx: number) => {
  ...
  if (activateWorkflows.value.some((activateWorkflow) => activateWorkflow.id === workflow.id)) {
    ...
    if (activateWorkflows.value.length >= 5) {
      ElMessage.warning('关联工作流已超过5个，无法继续关联')
      return
    }
    // 7.4 添加数据到激活工作流列表
...
<template>
  <div class="">
    <el-collapse-item name="workflows" class="app-ability-item">
      <template #title>
        <div class="flex w-full items-center justify-between gap-2 pr-2">
          <div class="flex min-w-0 items-center text-sm font-bold leading-none text-gray-700">工作流</div>
    ...
    <!-- 工作流模态窗 -->
    <el-dialog
      v-model="workflowsModalVisible"
      header-class="hidden"
      :show-close="false"
      :width="400"
      class="workflows-modal"
      modal-class="h-[calc(100vh-32px)] right-4"
      @close="handleCancelWorkflowsModal"
    >
      <!-- 顶部标题 -->
      ...
      <div class="flex items-center justify-between">
        ...
        <!-- 按钮组 -->
        <el-space :size="12">
          <el-button class="rounded-lg" @click="handleCancelWorkflowsModal">取消</el-button>
          <el-button

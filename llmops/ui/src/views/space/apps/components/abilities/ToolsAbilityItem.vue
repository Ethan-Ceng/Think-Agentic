...
const handleSelectTool = async (provider_idx: number, tool_idx: number) => {
  ...
  if (
    ...
    if (props.tools.length >= 5) {
      ElMessage.warning('一个Agent应用最多关联5个扩展插件')
      return
    }
...
<template>
  <div class="">
    <!-- 折叠面板 -->
    <el-collapse-item name="tools" class="app-ability-item">
      <template #title>
        <div class="flex w-full items-center justify-between gap-2 pr-2">
          <div class="flex min-w-0 items-center text-sm font-bold leading-none text-gray-700">扩展插件</div>
        ...
        >
          ...
          <div class="flex items-center gap-2">
            ...
            <el-avatar
              ...
              :src="resolveApiAssetUrl(tool.provider.icon)"
            />
            <!-- 名称与描述信息 -->
    ...
    >
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center">
          <!-- 工具信息 -->
          <div class="flex items-center gap-2">
            <el-avatar :size="24" shape="circle" :src="resolveApiAssetUrl(toolInfo?.provider?.icon)" />
            <div class="text-gray-700 font-bold max-w-[200px] line-clamp-1 break-all">
              {{ toolInfo?.tool?.label }}
            </div>
          </div>
          <!-- 分隔符 -->
          <div class="mx-4 text-gray-400 inline-flex items-center justify-center">
            <icon-oblique-line :size="12" class="-rotate-[52deg]" />
          </div>
          <!-- 导航菜单 -->
        ...
        <!-- 右侧关闭按钮 -->
        <el-button size="small" link class="text-gray-700!" @click="handleCancelToolInfoModal">
          <template #icon>
            <icon-close />
          </template>
        </el-button>
      ...
      <div
        ...
        class="h-[calc(100vh-170px)] pb-4 overflow-scroll scrollbar-y-sleek"
      >
        <!-- 工具描述 -->
      ...
      <div
        ...
        class="h-[calc(100vh-170px)] pb-4 overflow-scroll scrollbar-y-sleek"
      >
        <el-form v-model:model="toolInfoSettingForm" label-position="top" class="">
          <el-form-item
          ...
          >
            <template #label>
              <div class="flex items-center gap-1">
                ...
                <el-tooltip :content="param.label">
                  <icon-info-circle />
                </el-tooltip>
              </div>
            </template>
            <el-select
              ...
              v-model="toolInfoSettingForm[param.name]"
              placeholder="请输入参数值"
      ...
      <div class="flex items-center justify-between">
        <div class=""></div>
        <el-space :size="12">
          <el-button class="rounded-lg" @click="handleCancelToolInfoModal">取消</el-button>
          <el-button
            :loading="updateDraftAppConfigLoading"
            type="primary"
            class="rounded-lg"
    ...
    >
      <div class="flex w-full h-full">
        <!-- 左侧导航菜单 -->
        <div
          class="flex flex-col shrink-0 bg-gray-50 w-[200px] h-full px-3 py-4 overflow-scroll scrollbar-y-sleek"
        >
          <!-- 标题 -->
          <div class="text-gray-900 font-bold text-lg mb-4">关联插件</div>
          <!-- 添加插件按钮 -->
          <router-link :to="{ name: 'space-tools-list', query: { create_type: 'tool' } }">
            <el-button type="primary" class="w-full rounded-lg mb-5">创建自定义插件</el-button>
          </router-link>
          <!-- 工具类别导航 -->
        ...
        <div class="flex-1 p-4">
          ...
          <div class="w-full flex items-center justify-between gap-2 mb-7">
            ...
            <el-button size="small" link class="text-gray-700! ml-6" @click="toolsModalVisible = false">
              <template #icon>
                <icon-close />
              </template>
            </el-button>
          ...
          <div
            ...
            class="h-[calc(100vh-130px)] overflow-scroll scrollbar-y-sleek"
          >
            <div
                ...
                >
                  ...
                  <div class="flex items-center gap-2">
                    <el-avatar
                      :size="20"
                      shape="circle"
                      :src="`${apiPrefix}/builtin-tools/${builtin_tool.name}/icon`"
                    />
          ...
          <div v-if="toolsActivateType === 'api_tool'">
            <div
              ...
              class="block h-[calc(100vh-130px)] overflow-scroll scrollbar-y-sleek"
              @scroll="handleScroll"
            >
              <div
              ...
              >
                ...
                <div class="flex flex-col gap-1">
                  ...
                  >
                    <!-- 工具信息 -->
                    <div class="flex items-center gap-2">
                      <el-avatar :size="20" shape="circle" :src="resolveApiAssetUrl(api_tool_provider.icon)" />
...
<style>
.el-dialog.tool-setting-modal {
  text-align: right;
}

.tools-modal.el-dialog .el-dialog__body {
  padding: 0;
  height: 100%;
  width: 100%;
  border-radius: 8px;

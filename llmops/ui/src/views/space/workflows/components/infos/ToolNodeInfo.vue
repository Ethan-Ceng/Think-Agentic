  ...
  >
    ...
    <div class="flex items-center justify-between gap-3 mb-2">
      ...
      <div class="flex items-center gap-1 flex-1">
        <el-avatar :size="30" shape="square" class="bg-orange-500 rounded-lg shrink-0">
          <icon-tool />
        </el-avatar>
        <el-input
          v-model="form.title"
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2 text-gray-700 font-semibold">
            <div class="">绑定插件</div>
            <el-tooltip content="为工具节点绑定指定的扩展插件，支持插件广场以及自定义API插件。">
              <icon-question-circle />
            </el-tooltip>
          </div>
          <!-- 右侧绑定工具按钮 -->
          <el-button type="text" size="mini" class="text-gray-700!" @click="handleShowToolsModal">
            <template #icon>
              <icon-plus />
            </template>
          </el-button>
        ...
        <div v-if="form.tool?.provider?.id && form.tool?.tool?.name" class="flex flex-col gap-1">
          <div
            class="flex items-center justify-between bg-white p-3 rounded-lg cursor-pointer hover:shadow-xs group border"
          >
            <!-- 左侧工具信息 -->
        ...
        <div v-for="(input, idx) in form?.inputs" :key="idx" class="flex items-center gap-1">
          <div class="w-[30%] shrink-0">
            <div class="flex items-center gap-1 text-xs text-gray-500">
              ...
              <div class="text-gray-500 bg-gray-200 px-1 py-0.5 rounded-sm">{{ input.type }}</div>
            </div>
          </div>
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2 text-gray-700 font-semibold">
            ...
            <el-tooltip content="内置工具使用的PARAMS参数，用于初始化内置工具。">
              <icon-question-circle />
            </el-tooltip>
          </div>
        </div>
        ...
        <div v-for="(param, idx) in form?.params" :key="idx" class="flex items-center gap-1">
          ...
          <div class="w-[80%] shrink-0">
            <el-input size="mini" v-model="param.value" placeholder="请输入参数值" />
          </div>
        </div>
        <!-- 空数据状态 -->
        <el-empty v-if="form?.params.length <= 0" class="my-4">该工具暂无PARAMS数据</el-empty>
      </div>
      <el-divider class="my-4" />
      <!-- 输出参数 -->
      <div class="flex flex-col gap-2">
        ...
        <div v-for="(output, idx) in form?.outputs" :key="idx" class="flex flex-col gap-2">
          <div class="flex items-center gap-2">
            ...
            <div class="text-gray-500 text-xs bg-gray-200 px-1 py-0.5 rounded-sm">
              {{ output.type }}
            </div>
          </div>
        </div>
              ...
              >
                ...
                <div class="flex flex-col gap-1">
                  ...
                  >
                    <!-- 工具信息 -->
                    <div class="flex items-center gap-2">
                      <el-avatar :size="20" shape="circle" :src="api_tool_provider.icon" />

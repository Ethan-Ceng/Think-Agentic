  ...
  >
    ...
    <div class="flex items-center justify-between gap-3 mb-2">
      ...
      <div class="flex items-center gap-1 flex-1">
        <el-avatar :size="30" shape="square" class="bg-violet-500 rounded-lg shrink-0">
          <icon-storage />
        </el-avatar>
        <el-input
          v-model="form.title"
    ...
    <el-form ref="formRef" size="mini" :model="form" label-position="top" @submit.prevent="onSubmit">
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2 text-gray-700 font-semibold">
            <div class="">检索设置</div>
            <el-tooltip content="配置知识库的检索规则，支持相似性检索、混合检索、全文检索。">
              <icon-question-circle />
            </el-tooltip>
          </div>
        </div>
        <el-radio-group v-model="form.retrieval_config.retrieval_strategy">
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2 text-gray-700 font-semibold">
            ...
            <el-tooltip content="配置知识库的最大召回数量，范围为0-10.">
              <icon-question-circle />
            </el-tooltip>
          </div>
        </div>
        <div class="flex items-center gap-4 w-full pl-3">
          <el-slider v-model="form.retrieval_config.k" :step="1" :min="1" :max="10" />
          <el-input-number
            size="mini"
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2 text-gray-700 font-semibold">
            <div class="">最小匹配度</div>
            <el-tooltip content="配置知识库的最小匹配度，范围为0-1">
              <icon-question-circle />
            </el-tooltip>
          </div>
        </div>
        <div class="flex items-center gap-4 w-full pl-3">
          <el-slider v-model="form.retrieval_config.score" :step="0.01" :min="0" :max="0.99" />
          <el-input-number
            size="mini"
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2 text-gray-700 font-semibold">
            <div class="">关联知识库</div>
            <el-tooltip content="绑定知识库检索节点需要检索的知识库，最多可以关联5个知识库。">
              <icon-question-circle />
            </el-tooltip>
          </div>
          <!-- 右侧关联知识库按钮 -->
    ...
    <el-dialog
      ...
      @close="() => (datasetsModalVisible = false)"
    >
      <!-- 顶部标题 -->
      ...
      <div class="h-[calc(100vh-180px)] mb-4 overflow-scroll scrollbar-y-sleek">
        <div
          v-loading="getDatasetsWithPageLoading"
      ...
      <div class="flex items-center justify-between">
        ...
        <!-- 按钮组 -->
        <el-space :size="12">
          <el-button type="primary" class="rounded-lg" @click="() => (datasetsModalVisible = false)">
            确定
          </el-button>
        </el-space>
      </div>

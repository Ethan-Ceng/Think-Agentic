...
<template>
  <div
    v-if="props.visible"
    class="absolute right-0 top-0 bottom-0 w-[400px] bg-white z-50 border-l overflow-scroll scrollbar-y-sleek p-4"
  >
    <!-- 调试面板标题 -->
    ...
    <!-- tab面板 -->
    <el-tabs v-model="activatedTab">
      <el-tab-pane label="输入" name="input">
        <!-- 无输入数据样式 -->
        <el-empty v-if="inputs.length <= 0" class="my-4">该工作流暂无输入数据</el-empty>
        <!-- 有数据的UI -->
        <el-form
          ref="formRef"
          :model="form"
          ...
          >
            <template #label>
              <div class="flex items-center gap-2">
                ...
                <div class="text-xs text-gray-500 bg-gray-200 px-1 py-0.5 rounded-sm shrink-0">
                  {{ input.type }}
                </div>
              </div>
            </template>
            ...
            <el-input-number
              ...
              class="rounded-lg!"
            />
            <el-radio-group v-else-if="input.type === 'boolean'" v-model="form[input.name]">
      ...
      <el-tab-pane label="输出" name="output">
        ...
        <!-- 空数据状态 -->
        <el-empty v-if="!debugWorkflowLoading && !outputs && !debugWorkflowError" class="my-4">
          该工作流暂无运行调试结果
        </el-empty>
      </el-tab-pane>
    </el-tabs>

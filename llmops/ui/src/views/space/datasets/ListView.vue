...
const { loading, loadingMore, datasets, paginator, loadDatasets } = useGetDatasetsWithPage()
...
<template>
  ...
  >
    ...
    <el-row :gutter="20" class="flex-1">
      <!-- 有数据的UI状态 -->
      <el-col v-for="dataset in datasets" :key="dataset.id" :span="6" class="mb-5">
        <el-card shadow="hover" class="cursor-pointer rounded-lg transition-shadow">
          <!-- 顶部知识库名称 -->
          <div class="flex items-center gap-3 mb-3">
            <!-- 左侧图标 -->
            <el-avatar :size="40" shape="square" :src="dataset.icon" />
            <!-- 右侧知识库信息 -->
            <div class="flex flex-1 justify-between">
              ...
              <el-dropdown placement="bottom-end">
                ...
                <template #dropdown><el-dropdown-menu>
                  <el-dropdown-item @click="() => handleUpdate(dataset.id)">设置</el-dropdown-item>
                  <el-dropdown-item
                    class="text-red-500!"
      ...
      <!-- 没数据的UI状态 -->
      <el-col v-if="datasets.length === 0" :span="24">
        <el-empty
    ...
    <el-dialog
      :width="520"
      :model-value="props.createType === 'dataset' || showUpdateModal"
      class="rounded-xl"
    ...
    >
      ...
      <div class="pt-6">
        <el-form ref="formRef" :model="form" @submit.prevent="handleSubmit" label-position="top">
          <el-form-item prop="fileList" :rules="[{ required: true, message: '知识库图标不能为空' }]">
            <el-upload
              :limit="1"

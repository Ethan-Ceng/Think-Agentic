...
const handleSubmit = async () => {
  ...
  if (query.value.trim() === '') {
    ElMessage.warning('用户提问不能为空')
    return
  }

  // 11.2 检测上次提问是否结束，如果没结束不能发起新提问
  if (webAppChatLoading.value) {
    ElMessage.warning('上一次提问还未结束，请稍等')
    return
  }
...
const triggerFileInput = () => {
  ...
  if (image_urls.value.length >= 5) {
    ElMessage.error('对话上传图片数量不能超过5张')
    return
  }
...
<template>
  <div class="flex min-h-screen h-full">
    <!-- 左侧会话记录 -->
    <div class="w-[240px] shrink-0 border-r border-gray-200 p-4 flex flex-col bg-white">
      <!-- 顶部应用信息 -->
      <div class="flex items-center gap-3 mb-8 shrink-0">
        <el-avatar :size="32" shape="square" :src="web_app?.icon" class="shrink-0" />
        <div class="flex-1 text-base font-semibold line-clamp-1 break-all text-gray-700">
          <el-skeleton :loading="getWebAppLoading" animation>
            <el-skeleton-item :rows="1" :line-height="32" :line-spacing="4" />
          </el-skeleton>
          {{ web_app?.name }}
        </div>
      </div>
      <!-- 新增会话 -->
      <el-button type="primary" class="w-full rounded-lg mb-6 shrink-0" @click="addConversation">
        <template #icon>
          <icon-edit />
        </template>
        新增会话
      ...
      <!-- 会话列表 -->
      <div class="flex-1 overflow-scroll scrollbar-y-sleek">
        <!-- 置顶会话 -->
        <div class="mb-4">
          <div class="text-gray-700 font-semibold mb-2">置顶会话</div>
          <!-- 空白骨架屏 -->
          <el-skeleton :loading="getWebAppConversationsLoading" animation>
            <el-skeleton-item :rows="2" :line-height="32" :line-spacing="4" />
          </el-skeleton>
          <!-- 会话列表 -->
            ...
            >
              ...
              <el-dropdown placement="bottom-end">
                ...
                <template #dropdown><el-dropdown-menu>
                  <el-dropdown-item @click="() => changeIsPinned(idx, true)">取消置顶</el-dropdown-item>
                  <el-dropdown-item @click="() => updateName(idx, true)">重命名</el-dropdown-item>
                  <el-dropdown-item class="text-red-700" @click="() => deleteConversation(idx, true)">
                    删除
                  </el-dropdown-item>
          ...
          <!-- 空会话列表 -->
          <el-empty v-if="!getWebAppConversationsLoading && pinned_conversations.length === 0">
            <template #image>
              <icon-empty :size="36" />
            </template>
            暂无置顶会话
          ...
          <div class="flex flex-col gap-1">
            ...
            >
              ...
              <el-dropdown placement="bottom-end">
                ...
                <template #dropdown><el-dropdown-menu>
                  <el-dropdown-item @click="() => changeIsPinned(idx, false)">置顶会话</el-dropdown-item>
                  <el-dropdown-item @click="() => updateName(idx, false)"> 重命名</el-dropdown-item>
                  <el-dropdown-item class="text-red-700" @click="() => deleteConversation(idx, false)">
                    删除
                  </el-dropdown-item>
                </el-dropdown-menu></template></el-dropdown>
            </div>
    ...
    <div class="flex-1 min-h-screen bg-white">
      ...
      >
        <dynamic-scroller
          ...
          :min-item-size="1"
          @scroll="handleScroll"
          class="h-full scrollbar-y-sleek"
        >
        ...
        <div v-if="task_id && webAppChatLoading" class="h-[50px] flex items-center justify-center">
          <el-button :loading="stopWebAppChatLoading" class="rounded-lg px-2" @click="handleStop">
            <template #icon>
              <icon-poweroff />
            </template>
            停止响应
      ...
      >
        ...
        <div class="flex flex-col items-center gap-2">
          <el-avatar :size="48" shape="square" class="rounded-lg" :src="web_app?.icon" />
      ...
      <!-- 对话输入框 -->
      <div class="w-[600px] mx-auto flex flex-col shrink-0">
        <!-- 顶部输入框 -->

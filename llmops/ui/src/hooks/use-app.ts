...
export const useGetAppsWithPage = () => {
  // 1.定义hooks所需数据（首屏/刷新与滚动加载更多分开，避免整页遮罩一直挡在中间）
  const loading = ref(false)
  ...
  const loadApps = async (init: boolean = false, search_word: string = '') => {
    ...
    try {
      ...
      if (init) {
        apps.value = list as GetAppsWithPageResponse['data']['list']
      } else {
        apps.value.push(...(list as GetAppsWithPageResponse['data']['list']))
      }
    } finally {
      loading.value = false
  ...
  return { loading, loadingMore, apps, paginator, loadApps }
}
...
export const useGetDebugConversationMessagesWithPage = () => {
  ...
  const loadDebugConversationMessages = async (app_id: string, init: boolean = false) => {
    ...
    if (init) {
      ...
      (paginator.value?.current_page ?? 0) > (paginator.value?.total_page ?? 0)
    ) {
      return
    }
    ...
    try {
      ...
      // 2.3 更新分页器（无 paginator 时保留原状态，避免读写 undefined）
      const nextPaginator = data.paginator
      if (nextPaginator && typeof nextPaginator.current_page === 'number') {
        paginator.value = nextPaginator
        const p = paginator.value
        if (
          typeof p.total_page === 'number' &&
          p.current_page <= p.total_page
        ) {
          paginator.value = { ...p, current_page: p.current_page + 1 }
        }
      }
      ...
      if (init) {
        ...
        created_at.value = list[0]?.created_at ?? 0
      }
    } finally {
      loading.value = false
    }
...
export const useRegenerateWebAppToken = () => {
  ...
  const handleRegenerateWebAppToken = async (app_id: string) => {
    try {
      ...
      ElMessage.success('重新生成WebApp访问链接成功')

...
def test_web_app_routes_keep_legacy_paths() -> None:
    account = Account(id=uuid.uuid4(), name="tester", email="tester@example.test")

    class FakeWebAppService:
        def get_web_app_info(self, session, token):  # noqa: ANN001
            assert token == "token"
            return {"id": str(uuid.uuid4()), "name": "App"}

        def get_conversations(self, session, token, is_pinned, current_user):  # noqa: ANN001
            assert token == "token"

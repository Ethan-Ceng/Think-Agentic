...
| 2026-07-25 | `READY_TO_MERGE` | 无 | 用户在页面复验提示后确认继续；当前 master 上前端 72 项、类型检查和生产构建重新通过 |
...
用户后续页面验收发现侧栏长标题/无断点最近消息会把会话 item 按内容宽度撑开，右侧时间和更多操作区随后被列表横向裁剪。新增 CSS 布局契约回归先在旧样式上失败；列表壳、分组和 item 增加明确的宽度与 `min-width: 0` 约束，标题文字参与 flex 收缩并显示省略号。整改后目标测试、前端 72 项全量测试、类型检查和生产构建通过；用户在页面复验提示后确认继续，视觉验收闭环。
...
- 用户后续验收：发现侧栏长文本会遮出操作区；布局约束整改后，用户在页面复验提示后确认继续。
...
`READY_TO_MERGE`：侧栏长文本布局修复的回归测试、前端全量测试、类型检查、生产构建和用户页面复验均通过，代码审查 `APPROVED`，无未处理 blocking/major。修复已由用户提交至 `master` 的 `24c09e0`。

...
- 用户后续验收发现侧栏长文本会把操作区裁出可见范围；CSS 收缩约束和回归测试已补齐，但本轮没有可用浏览器控制实例，最终视觉复验等待用户刷新页面确认。
...
| 验收标准满足 | 待视觉复验 | 原十项验收已通过；用户后续发现的侧栏长文本布局已整改，等待刷新确认 |
...
- 剩余风险：同一 Agent 自检、未执行压力测试，以及本轮浏览器实例不可用导致长文本布局尚待用户最终目测。
- 下一步：用户刷新并确认侧栏长文本省略且更多操作按钮可见后，将计划恢复为 `READY_TO_MERGE`。

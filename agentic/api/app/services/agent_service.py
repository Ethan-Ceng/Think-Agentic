...
class AgentService:
    ...
    async def chat(
        ...
        try:
            ...
            # 10.5 空消息 + 无任务时，保持长连接等待（避免前端立即重连形成循环）
            # 设置最长等待时间，超时后让前端重连
            if not message and task is None:
                logger.debug(f"会话[{session_id}]无任务，空流等待中...")
            ...
            while task:
                ...
                # 14.将未读消息数重置为0
                if not isinstance(event, (MessageDeltaEvent, ExecutionUpdateEvent)):
                    async with self._uow:
